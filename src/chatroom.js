...
function Chatroom() {
  ...
  const sendMessage = async (message) => {
    try {
      ...
      const response = await axios.post('http://3.35.151.0:5000/query/NORMAL/Sejong', {
        query: message,
      });
  ...
  const handleQuizButtonClick = async () => {
    ...
    try {
      const response = await axios.post('http://3.35.151.0:5000/query/QUIZ/Sejong', {
        BotType: 'QUIZ',
      });
      const data = response.data;
